...
st.title("Subtitle to Organized Text Podacst")
...
SYSTEM_PROMPT = '''Below is the transcript of a video. Please correct the capitalization and punctuation, including making separate paragraphs, without changing any of the text. If a word is misspelled, correct the word, and if a word does not exist take your best guess as to the correct word. Only return the corrected text without commentary:'''
video_prompt = '[INST] Generate Summeries for each of the discussed topics in the text provided based on sentiment. Ensure each summery has its own Paragraph. [/INST]'
...
st.title("YouTube Subtitle, Punctuation, and Discussion Summary Generator")
...
if st.session_state.subtitles:  # If raw subtitles are available in the session state
    if st.button("Punctuate Script"):
        # Pass subtitles to Prompt
        # prompt = st.session_state.subtitles
        tokens = nltk.word_tokenize(st.session_state.subtitles)

        if len(tokens) > MAX_TOKENS:

            chunks = []
            current_len = len(SYSTEM_PROMPT.split())

            for i in range(0, len(tokens), MAX_TOKENS - current_len):
                chunk = ' '.join(tokens[i:i+MAX_TOKENS-current_len])
                chunks.append(chunk)

            completions = []

            for chunk in chunks:
                st.session_state.formatted_text = format_with_clarifai_api(chunk, SYSTEM_PROMPT)
    ...
    if st.session_state.formatted_text:
        st.text_area("Formatted Subtitles:", value=output, height=400)
        
        # Check for the st.session_state.formatted_text before showing the "Generate title and description" button
        if st.button("Generate Summeries"):
            # Generate title and description
            video_description = format_with_clarifai_api(output, video_prompt)
